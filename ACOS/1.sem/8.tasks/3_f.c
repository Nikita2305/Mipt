...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < files; i++) {
        ...
        // printf("elements size, %d", elements_size[i]);
        close(fd);
    }
    ...
    while (1) {
        int id = -1;
        for (int i = 0; i < files; i++) {
            if (elements_ptr[i] < elements_size[i] &&
                (id == -1 || elements[id][elements_ptr[id]] >=
                                 elements[i][elements_ptr[i]])) {
                id = i;
            }
        }
        if (id == -1) {
            write(
                fd_out,
                buffer,
                buffer_ptr * sizeof(int)); // TODO: change 1 -> output_fd
            break;
        }
        ...
        if (elements_ptr[id] == elements_size[id]) {
            ...
            int read_bytes =
                read(fd, elements[id], mini_bucket_size * sizeof(int));
