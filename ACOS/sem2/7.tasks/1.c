...
int main(int argc, char** argv) {
    ...
    while (1) {
        ...
        if ((now.tv_nsec - start.tv_nsec) / 1000.0 + (now.tv_sec - start.tv_sec) * 1000000.0 >
        ...
        if (-1 == sendto(sfd, &packet, sizeof(struct send_packet), 0, (struct sockaddr*)&addr, addr_len)) {
            return 1;
        }

        if (-1 == recvfrom(sfd, &packet, sizeof(struct send_packet), 0, (struct sockaddr*)&addr, &addr_len)) {
            return 1;
        }
